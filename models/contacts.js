...
const getContactById = async (contactId) => {
  ...
  const singleContact = contacts.filter((item) => item.id === contactId);
...
const updateContact = async (contactId, body) => {
  const contacts = await getAllContacts();
  const userIndex = contacts.findIndex((item) => item.id === contactId);
 const updatedContact = { ...contacts[userIndex], ...body };
